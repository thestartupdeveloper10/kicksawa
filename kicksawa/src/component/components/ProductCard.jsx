...
const ProductCard = ({ id, image, category, name, price }) => {
  ...
  return (
    <div className={`border ${theme === 'dark' ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'} p-4 flex flex-col transition-colors`}>
      <div className="relative mb-4">
        <Link to={`/products/${id}`}>
          <img src={image} alt={name} className="w-full h-48 object-cover" />
        </Link>
        <div className="absolute z-50 top-2 right-2 flex space-x-2">
          <button className={`p-1 ${theme === 'dark' ? 'bg-gray-700' : 'bg-white'} rounded-full shadow`}>
